...
const sections = [
  ...
  ['Synchronize', 'After a verified Make run, import source-backed Hyderabad business records and see duplicates and rejected rows.', 'The current screen says Setup Required because no approved source connection is active yet. Do not run the old Apify flow.'],
...
export default function Training() {
  return <div className="sw-root">
    ...
    <section className="sw-panel"><div className="sw-row"><span className="sw-pill"><RefreshCw size={14} /> Live source status</span><Link to="/make-scenarios">Open Synchronize →</Link></div><h2>What must be connected before live leads arrive?</h2><p>Google business data requires an approved Google source connection. LinkedIn data must come from an official LinkedIn export, Lead Gen Form, or approved API access. LinkedIn scraping and automatic connection/message sending are not safe or supported.</p><p className="sw-muted">Once the approved source is connected in Make, the flow is: source → Hyderabad/business filter → duplicate check → CRVM → salesperson call guide → daily report.</p></section>
  </div>;
}
